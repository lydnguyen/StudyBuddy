...
def start_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
def help_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Send a message when the command /help is issued."""
    update.message.reply_text("Type /start")


def new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
def main_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
def poll_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def add_typing(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def add_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE, message):
    await context.bot.send_message(chat_id=get_chat_id(update, context), text=message)


async def add_suggested_actions(update: Update, context: ContextTypes.DEFAULT_TYPE, response):
    options = []
...
async def add_quiz_question(update: Update, context: ContextTypes.DEFAULT_TYPE, quiz_question):
...
async def add_poll_question(update: Update, context: ContextTypes.DEFAULT_TYPE, quiz_question):
...
def get_text_from_message(update: Update):
...
def is_answer_correct(update: Update):
    answers = update.poll.options
...
def get_text_from_callback(update: Update):
...
def main():
    ...
    dp = ApplicationBuilder().token(DefaultConfig.TELEGRAM_TOKEN).build()
    # command handlers
